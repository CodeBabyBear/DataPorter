...
import com.newegg.eims.DataPorter.Spark.SparkHiveMetastoreTypes.DecimalTypeFixed
import org.apache.avro.Schema
import org.apache.hadoop.conf.Configuration
import org.apache.hadoop.fs.Path
import org.apache.hadoop.hive.common.`type`.{HiveChar, HiveDecimal, HiveVarchar}
import org.apache.hadoop.hive.ql.io.orc.{OrcOutputFormat, OrcSerde, OrcStruct}
...
import org.apache.hadoop.hive.serde2.typeinfo.{DecimalTypeInfo, StructTypeInfo, TypeInfoFactory, TypeInfoUtils}
import org.apache.hadoop.hive.serde2.{ColumnProjectionUtils, io => hiveIo}
...
import org.apache.spark.sql.{AnalysisException, Row, types}
...
/**
  ...
  * Creator: vq83
  */
class SparkOrcOutputWriter(
                            path: Path,
                            dataSchema: StructType,
                            conf: JobConf)
  extends OutputWriter with SparkHiveInspectors {

  private val serializer = {
    val table = new Properties()
    table.setProperty("columns", dataSchema.fieldNames.mkString(","))
    table.setProperty("columns.types", dataSchema.map { f =>
      SparkHiveMetastoreTypes.toMetastoreType(f.dataType)
    }.mkString(":"))
  ...
  private val structOI = {
    val typeInfo =
      TypeInfoUtils.getTypeInfoFromTypeString(
        SparkHiveMetastoreTypes.toMetastoreType(dataSchema))
...
private[Spark] object SparkHiveShim {
   ...
   */
  implicit def wrapperToFileSinkDesc(w: SparkShimFileSinkDesc): FileSinkDesc = {
   ...
   * Fix it through wrapper.
   */
  private[Spark] class SparkShimFileSinkDesc(
                                              var dir: String,
...
private[Spark] object SparkHiveMetastoreTypes {
  ...
  def decimalMetastoreString(decimalType: DecimalType): String = decimalType match {
    case DecimalTypeFixed(precision, scale) => s"decimal($precision,$scale)"
    case _ => s"decimal($SparkHiveShim.UNLIMITED_DECIMAL_PRECISION,$SparkHiveShim.UNLIMITED_DECIMAL_SCALE)"
  }

  private[Spark] object DecimalTypeFixed {
...
/**
  ...
  */
private[Spark] trait SparkHiveInspectors {
  ...
  def javaClassToDataType(clz: Class[_]): DataType = clz match {
    ...
    case c => throw new Exception(s"Unsupported java type $c")
  }
  ...
  def unwrap(data: Any, oi: ObjectInspector): Any = oi match {
    ...
    case poi: WritableConstantHiveDecimalObjectInspector =>
      SparkHiveShim.toCatalystDecimal(
    ...
    case pi: PrimitiveObjectInspector => pi match {
      ...
      case x: HiveDecimalObjectInspector => SparkHiveShim.toCatalystDecimal(x, data)
  ...
  private def getStringWritable(value: Any): hadoopIo.Text =
    if (value == null) null else new hadoopIo.Text(value match {
      case string: UTF8String => string.getBytes
      case string:String => UTF8String.fromString(string).getBytes
      case x  =>  UTF8String.fromString(x.toString).getBytes
    })
  ...
  implicit class typeInfoConversions(dt: DataType) {
    ...
    private def decimalTypeInfo(decimalType: DecimalType): TypeInfo = decimalType match {
      case DecimalTypeFixed(precision, scale) => new DecimalTypeInfo(precision, scale)
    }
